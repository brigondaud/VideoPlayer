 ...
 */
pthread_mutex_t mutex_draw2SDL;
...
void *draw2SDL(void *arg)
{
	...
	pthread_mutex_lock(&mutex_draw2SDL);
	HASH_FIND_INT(theorastrstate, &serial, s);
	pthread_mutex_unlock(&mutex_draw2SDL);
	...
	while (!fini) {
		...
		int delaims =
		    (int)(texturedate[tex_iaff].timems - timemsfromstart);
